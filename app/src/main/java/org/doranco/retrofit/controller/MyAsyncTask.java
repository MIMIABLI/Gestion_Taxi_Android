package org.doranco.retrofit.controller;

import android.os.AsyncTask;
import android.util.Log;
import org.doranco.models.Client;
import org.doranco.retrofit.RetrofitService;
import org.doranco.retrofit.interfacesapi.ClientApi;
import retrofit2.Call;

import java.io.IOException;
import java.lang.ref.WeakReference;

public class MyAsyncTask extends AsyncTask<Void, Void, Call<Client>> {

        RetrofitService retrofitService = new RetrofitService();
        ControllerClient controllerClient;
        ClientApi clientApi;
        String login;

        // 1 - Implement listeners methods (Callback)
        public interface Listeners {
            void onPreExecute();
            void doInBackground();
            void onPostExecute(Call<Client> success);
        }

        // 2 - Declare callback
        private final WeakReference<Listeners> callback;

        // 3 - Constructor
        public MyAsyncTask(Listeners callback, String login, ControllerClient controllerClient){
            this.callback = new WeakReference<>(callback);
            this.login = login;
            this.controllerClient = controllerClient;
            this.clientApi = retrofitService.getRetrofit().create(ClientApi.class);
        }

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            this.callback.get().onPreExecute(); // 4 - Call the related callback method
            Log.e("TAG", "AsyncTask is started.");
        }

        @Override
        protected void onPostExecute(Call<Client> success) {
            super.onPostExecute(success);
            this.callback.get().onPostExecute(success); // 4 - Call the related callback method
            Log.e("TAG", "AsyncTask is finished.");
            System.out.println("Is Client exists? -> " + controllerClient.getClientExistance());
            System.out.println(controllerClient.getClientLogin());
        }

        @Override
        protected Call<Client> doInBackground(Void... voids) {
            this.callback.get().doInBackground(); // 4 - Call the related callback method
            Log.e("TAG", "AsyncTask doing some big work...");
            try {
                return controllerClient.getClientDatas(login); // 5 - Execute our task
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }
    }

